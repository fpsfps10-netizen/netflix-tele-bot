...
module.exports = async (req, res) => {
    try {
        ...
        if (req.body && req.body.to && req.body.content) {
            ...
            // البحث عن الزبائن المربوطين بهذا الإيميل
            const targetUsers = data.users.filter(u => u.email === emailTo);

            // أ- استخراج الكود المكون من 4 أرقام فقط (يتجاهل الـ 5 أرقام مثل 95032)
            const codeMatch = content.match(/\b\d{4}\b/g);
            const validCode = codeMatch ? codeMatch[codeMatch.length - 1] : null;

            // ب- استخراج رابط "Get Code" المباشر من نيتفليكس
            const linkMatch = content.match(/https:\/\/www\.netflix\.com\/[^\s"']+/);
            const getCodeLink = linkMatch ? linkMatch[0] : null;

            // ج- الكلمات المفتاحية لتحديد إيميلات الـ Household والوصول المؤقت
            ...
            for (const user of targetUsers) {
                ...
                // حالة وصول الكود الرقمي الصريح (4 أرقام)
                if (validCode) {
                    await bot.telegram.sendMessage(user.id, 
        ...
        // --- 2. معالجة أوامر البوت من قبل المستخدمين ---
